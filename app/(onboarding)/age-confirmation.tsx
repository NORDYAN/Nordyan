...
export default function AgeConfirmationScreen() {
  const { locale } = useI18n();
  const copy = useMemo(() => AGE_CONFIRMATION_COPY, [locale]);
  const [checked, setChecked] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const canContinue = canSubmitAgeConfirmation(checked);
        ...
        <View style={styles.stage}>
          ...
          <View style={styles.footer}>
            <Button
              ...
              style={[styles.button, (!canContinue || isSubmitting) && styles.buttonDisabled]}
              onPress={() => {
                void handleContinue();
              }}
            />
